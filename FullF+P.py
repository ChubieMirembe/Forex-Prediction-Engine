...
def bigChart(dt):
    ...
    X = np.reshape(dt.index, (120,-1))        
    regressor_test.fit(X, dt['Close'])
    yi = regressor_test.predict(X)
    dt['Distance'] = dt['Close'].values - yi   
    res = dt['Distance'].values.reshape(-1,1)
    dt['Distance'] = scaler.fit_transform(res)    
    plt.bar(dt.index, dt['Distance'])
    plt.show()
    
    return dt

def showPatterns(pattern_test):
...
def searching(pattern_test, features, features_test, pattern):
    test_pattern = []
    dist_list = []
    pattern_number = []
    ...
    return s_path, p_numb, dataset

    
def compare(dataset, pattern, p_numb, s_path, pattern_test, dataset_test ):
...
def test(s):
    ...
    pt = findPatterns(ndt)
    fea = featuresT(pt)
    sea = searching(pt, allFeatures, fea, AP)
    compare(sea[2], AP, sea[1], sea[0], pt, dt)
